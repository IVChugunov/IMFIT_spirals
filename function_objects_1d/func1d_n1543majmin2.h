...
// CLASS n1543MajMin21D:

#include "function_object.h"


class n1543MajMin21D : public FunctionObject
{
  ...
  private:
    double  x0, n, mu_e, r_e_maj, r_e_min, mu_0_bar, h_bar1, h_bar2, r_b, alpha;   // parameters
    double  mu_0_disk, h_disk_maj, h_disk_min, sigma_bar, mu_0_nsc, sigma_nsc;   // more parameters
    double  I_e, n2, bn, invn, I_0_disk, I_0_bar, exponent, S, I_0_nsc;   // other useful quantities

	double  Gaussian( double x, double I_0, double sigma );
	double  Exponential( double x, double I_0, double h );
	double  Sersic( double x, double nn, double Ie, double re );
