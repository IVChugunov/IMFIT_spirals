...
const char FUNCTION_NAME[] = "n1543majmin-1D function";
#define CLASS_SHORT_NAME  "n1543majmin-1D"

const char n1543MajMin1D::className[] = CLASS_SHORT_NAME;


/* ---------------- CONSTRUCTOR ---------------------------------------- */

n1543MajMin1D::n1543MajMin1D( )
{
  string  paramName;
...
void n1543MajMin1D::Setup( double params[], int offsetIndex, double xc )
{
  x0 = xc;
...
double n1543MajMin1D::GetValue( double x )
{
  double  r = fabs(x - x0);
  double  I = 0.0;
...
double n1543MajMin1D::Gaussian( double x, double I_0, double sigma )
{
  // We assume that x >= 0
...
double n1543MajMin1D::Exponential( double x, double I_0, double h )
{
  // We assume that x >= 0
  return (I_0 * exp(-x/h));
}
...
double n1543MajMin1D::Sersic( double x, double nn, double Ie, double re )
{
  // We assume that x >= 0
