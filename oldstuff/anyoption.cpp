...
// UPDATE BY PETER ERWIN (Dec 2009):
// Commented out printVerbose() code at end of processCommandArgs(), because
// any legitimate command-line *argument* would generate the "Unknown command argument option"
// message.
...
AnyOption::parsePOSIX( char* arg )
{
	...
	printVerbose( "parsePOSIX: Unknown command argument option : " );
	printVerbose( arg );
	printVerbose( );
	printAutoUsage();
	return '0';
...
AnyOption::parseGNU( char *arg )
{
	...
	if( split_at > 0 ){ /* it is an option value pair */
		...
		if ( matchOpt( tmp ) >= 0 ){
			...
			printVerbose( "parseGNU: Unknown command argument option : " );
			printVerbose( arg );
			printVerbose( );
			printAutoUsage();
			free (tmp);
...
AnyOption::matchOpt( char *opt )
{
	...
	printVerbose( "matchOpt: Unknown command argument option : " );
	printVerbose( opt  ) ;
	printVerbose( );
	printAutoUsage();
	return  -1;	
...
//				return false;
	...
	printVerbose( "matchChar: Unknown command argument option : " );
	printVerbose( c ) ;
	printVerbose( );
	printAutoUsage();
	return false;
