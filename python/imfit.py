...
def ReadImfitConfigFile( fileName, minorAxis=False, pix=1.0, getNames=False, X0=0.0 ):
    ...
    if not os.path.exists(fileName):
        txt = "ReadImfitConfigFile: ERROR: input file \"{0}\" ".format(fileName)
        txt += "not found!"
        print(txt)
        return None
...
def GetBootstrapOutput( fileName ):
    ...
    if not os.path.exists(fileName):
        txt = "GetBootstrapOutput: ERROR: input file \"{0}\" ".format(fileName)
        txt += "not found!"
        print(txt)
        return None
...
def GetSingleChain( fileName, getAllColumns=False ):
    ...
    if not os.path.exists(fileName):
        txt = "GetSingleChain: ERROR: input file \"{0}\" ".format(fileName)
        txt += "not found!"
        print(txt)
        return None
...
def MergeChains( fname_root, maxChains=None, getAllColumns=False, start=10000, last=None,
    ...
    if maxChains is None:
        ...
        if len(filenames) == 0:
            txt = "MergeChains: ERROR: No MCMC output files found using "
            txt += "pattern \"{0}\"!".format(globPattern)
            print(txt)
            return None
    else:
    ...
    # check to see if any are missing (shouldn't happen for maxChains=None, since
    # we check the glob pattern above; might happend when maxChains != None)
    badFilenames = [ fname for fname in filenames if not os.path.exists(fname) ]
    if len(badFilenames) > 0:
        txt = "MergeChains: ERROR: one or more input files not found!\n\t"
        if len(badFilenames) == 1:
            txt += "\"{0}\"".format(badFilenames[0])
        else:
            for badFname in badFilenames[:-1]:
                txt += "\"{0}\", ".format(badFname)
            txt += "\"{0}\"".format(badFilenames[-1])
        print(txt)
        return None
