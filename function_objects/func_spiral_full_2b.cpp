...
/* ---------------- Definitions ---------------------------------------- */
const int N_PARAMS = 31;
const char PARAM_LABELS[][20] = {"PA", "ell", "r_0", "phi_0", "r_break_1", "phi_break_1", "r_break_2", "phi_break_2", "r_end", "phi_end",
                                 "mu_a_2", "mu_a_3", "mu_b_2", "mu_b_3", "mu_c_2", "mu_c_3",
                                 "I_0", "part_growth", "h_s", "part_cutoff",
                                 "w_zp", "w_i", "S_0", "S_end", "n",
                                 "A_d_1", "phi_d_1", "w_d_1", "A_d_2", "phi_d_2", "w_d_2"};

const char FUNCTION_NAME[] = "Spiral arm function with 2 breaks and extended parameters";
const double pi = 3.14159265358979323846;
const double pi2 = 6.28318530718;
const double DEG2RAD = 0.017453292519943295;

const char SpiralArmFull2b::className[] = "SpiralArmFull2b";


/* ---------------- CONSTRUCTOR ---------------------------------------- */

SpiralArmFull2b::SpiralArmFull2b() {
    string paramName;
...
void SpiralArmFull2b::Setup(double params[], int offsetIndex, double xc, double yc) {
    x0 = xc;
    y0 = yc;
    ...
    mu_a_3 = params[11 + offsetIndex];
    mu_b_2 = params[12 + offsetIndex];
    mu_b_3 = params[13 + offsetIndex];
    mu_c_2 = params[14 + offsetIndex];
    mu_c_3 = params[15 + offsetIndex];
    I_0 = params[16 + offsetIndex];
    part_growth = params[17 + offsetIndex];
    ih_s = params[18 + offsetIndex];
    part_cutoff = params[19 + offsetIndex];
    w_zp = params[20 + offsetIndex];
    w_i = params[21 + offsetIndex];
    S_0 = params[22 + offsetIndex];
    S_end = params[23 + offsetIndex];
    n = params[24 + offsetIndex];
    A_d_1 = params[25 + offsetIndex];
    phi_d_1 = params[26 + offsetIndex] * DEG2RAD;
    w_d_1 = params[27 + offsetIndex] * DEG2RAD;
    A_d_2 = params[28 + offsetIndex];
    phi_d_2 = params[29 + offsetIndex] * DEG2RAD;
    w_d_2 = params[30 + offsetIndex] * DEG2RAD;
    ...
    m_b_3 = 2 * mu_b_3;
    m_c_1 = mu_c_1 - mu_c_2 + mu_c_3;
    m_c_2 = mu_c_2 - 3 * mu_c_3;
    m_c_3 = 2 * mu_c_3;
...
double SpiralArmFull2b::GetValue(double x, double y) {
    double xc = x - x0;
    double yc = y - y0;
...
double SpiralArmFull2b::GetBrightness(double r, double psi) {
...
double SpiralArmFull2b::GetParallelBrightness(double r_spiral, double psi) {
...
double SpiralArmFull2b::GetNormalBrightness(double r_spiral, double rho, double psi) {
...
double SpiralArmFull2b::GetNearestCoordinates(double r, double psi) {
...
double SpiralArmFull2b::GetRadius(double psi) {
    double psi_norm;
    if (psi >= psi_break_2) {
        psi_norm = (psi - psi_break_2) / (psi_end - psi_break_2);
    	return r_break_2 * exp(m_c_1 * psi_norm + m_c_2 * pow(psi_norm, 2) + m_c_3 * pow(psi_norm, 3));
...
/* END OF FILE: func_spiral_full_2b.cpp ------------------------------------ */
