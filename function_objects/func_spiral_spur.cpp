...
const char PARAM_LABELS[][20] = {"PA", "ell", "r_0", "phi_0", "r_end", "phi_end", "mu_a_2",
                                 "I_0", "part_growth", "part_cutoff", "w_i"};

const char FUNCTION_NAME[] = "Spiral spur function";
const double pi = 3.14159265358979323846;
const double pi2 = 6.28318530718;
const double DEG2RAD = 0.017453292519943295;

const char SpiralSpur::className[] = "SpiralSpur";


/* ---------------- CONSTRUCTOR ---------------------------------------- */

SpiralSpur::SpiralSpur() {
    string paramName;
...
void SpiralSpur::Setup(double params[], int offsetIndex, double xc, double yc) {
    x0 = xc;
    y0 = yc;
    ...
    I_0 = params[7 + offsetIndex];
    part_growth = params[8 + offsetIndex];
    part_cutoff = params[9 + offsetIndex];
    w_i = params[10 + offsetIndex];
...
double SpiralSpur::GetValue(double x, double y) {
    double xc = x - x0;
    double yc = y - y0;
...
double SpiralSpur::GetBrightness(double r, double psi) {
...
double SpiralSpur::GetParallelBrightness(double r_spiral, double psi) {
    double I_par = 1.0;
...
double SpiralSpur::GetNormalBrightness(double r_spiral, double rho, double psi) {
    double loc_w = abs(w_i * r_spiral) / 2;
    return exp(-bn * pow(abs(rho) / loc_w, 2));
}

double SpiralSpur::GetNearestCoordinates(double r, double psi) {
