 ...
 *            [currently, I_rad(r) = constant = I_0]
...
const char  FUNCTION_NAME[] = "Logarithmic Spiral function";
const double  DEG2RAD = 0.017453292519943295;
const int  SUBSAMPLE_R = 10;
const double  MIN_RADIUS = 0.001;

const char LogSpiral::className[] = "LogSpiral";
...
void LogSpiral::Setup( double params[], int offsetIndex, double xc, double yc )
{
  x0 = xc;
  y0 = yc;
...
double LogSpiral::CalculateIntensity( double r, double phi )
{
  double  I, logSpiralFn;
...
// r is *non-negative*!
double LogSpiral::GetValue( double x, double y )
{
  double  x_diff = x - x0;
  double  y_diff = y - y0;
  double  xp, yp_scaled;
...
int LogSpiral::CalculateSubsamples( double r )
{
  int  nSamples = 1;
  return nSamples;
}
...
/* END OF FILE: func_logspiral.cpp ------------------------------------- */
